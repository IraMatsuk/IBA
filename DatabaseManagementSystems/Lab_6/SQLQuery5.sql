...
INSERT INTO ORDERS VALUES (113043,'2008-02-20',2113,101,'REI','2A44R',5,22500.00);
...
BEGIN
	BEGIN TRY
		INSERT INTO CUSTOMERS (CUST_NUM, COMPANY, CUST_REP, CREDIT_LIMIT) VALUES (@cust_num, @company, @cust_rep, @credit_limit)
	END TRY
	BEGIN CATCH
		SET @count = -1
		PRINT 'Ошибка! Вы уже добавили данного клиента.';
	END CATCH
	RETURN @count;
END
...
DECLARE @cust_num int, 
		...
		@credit_limit decimal(9,2);
SET @cust_num = 2130
SET @company = 'Matsuk Family'
SET @cust_rep = 110
SET @credit_limit = 70000
--Выполнение процедуры
EXEC AddCustomer @cust_num, @company, @cust_rep, @credit_limit;
...
begin
	...
	begin catch
		set @rc = -1
		print 'Error! This customer already exists!'
	end catch
	return @rc
...
go
--Вызываем процедуру без указания названия параметра credit_limit
EXECUTE add_cust @cust_num = 2132,
				 @company = 'New Company2',
				 @cust_rep = 107

--Вызываем процедуру с указанием названия всех параметров
EXEC add_cust @cust_num = 2132,
				 @company = 'New Company2',
				 @cust_rep = 107,
				 @credit_limit = 90000

--Вызываем процедуру без указания названия параметров
EXEC add_cust 2133, 'New Company3', 107, 5000
...
--DELETE CUSTOMERS WHERE CUST_NUM = 2132;
--DELETE CUSTOMERS WHERE CUST_NUM = 2133;


--1.2.Поиска клиента по части названия; если такого не нашлось — вывести сообщение. 
CREATE PROCEDURE FindCustomer @part_name_company varchar(5)
AS
BEGIN
	IF NOT EXISTS (SELECT COMPANY FROM CUSTOMERS WHERE COMPANY LIKE '%' + @part_name_company + '%')
	PRINT 'Совпадений не найдено.';
	ELSE
	SELECT COMPANY FROM CUSTOMERS WHERE COMPANY LIKE '%' + @part_name_company + '%'
END

GO

--вызов процедуры
EXEC FindCustomer @part_name_company = 'corp'; --5 совпадений
EXEC FindCustomer @part_name_company = 'qa'; -- совпадений не найдено
...
--можно ли отменить update?
CREATE PROCEDURE CustomerDataUpdates @cust_rep int
AS
DECLARE @rc int = 0;
BEGIN
	UPDATE CUSTOMERS
	SET CREDIT_LIMIT = 100000
	WHERE CUST_REP = @cust_rep;
END
GO

--вызов процедуры
--DECLARE @cust_rep int
EXEC CustomerDataUpdates @cust_rep = 102;  
EXECUTE CustomerDataUpdates @cust_rep = 100;
--DROP PROCEDURE CustomerDataUpdates
...
--компании и их заказы
SELECT C.COMPANY, O.ORDER_NUM
FROM CUSTOMERS C LEFT JOIN ORDERS O
ON C.CUST_NUM = O.CUST;

--компании у которых нет заказов
SELECT C.CUST_NUM, C.COMPANY, O.ORDER_NUM
FROM CUSTOMERS C LEFT JOIN ORDERS O
ON C.CUST_NUM = O.CUST
WHERE O.ORDER_NUM IS NULL;

CREATE PROCEDURE DeleteCustomerDate 
AS
BEGIN
	DECLARE @c int = (SELECT COUNT(C.CUST_NUM) FROM CUSTOMERS C LEFT JOIN ORDERS O 
		ON C.CUST_NUM = O.CUST
		WHERE O.ORDER_NUM IS NULL);
	IF @c <> 0
	BEGIN
		DELETE FROM CUSTOMERS WHERE CUST_NUM IN (SELECT CUST_NUM
			FROM CUSTOMERS C LEFT JOIN ORDERS O
			ON C.CUST_NUM = O.CUST
			WHERE O.ORDER_NUM IS NULL);
	END
...
--3.1.Подсчитать количество заказов сотрудника в определенный период. Если такого сотрудника нет — вернуть -1. Если сотрудник есть, а заказов нет — вернуть 0.
--SELECT * FROM SALESREPS
--SELECT * FROM ORDERS
drop function CountSalesrepsOrders

CREATE FUNCTION CountSalesrepsOrders (@rep int, @order_date_from date, @order_date_to date) returns int
BEGIN
	DECLARE @rc int = -1;
	IF (SELECT EMPL_NUM FROM SALESREPS WHERE EMPL_NUM = @rep) IS NULL
	RETURN @rc;

	SET @rc = (SELECT COUNT(ORDER_NUM) FROM ORDERS WHERE REP = @rep AND ORDER_DATE BETWEEN @order_date_from and @order_date_to);
	RETURN @rc;
END;
go
...
--use function
DECLARE @rep int;
SET @rep = [dbo].CountSalesrepsOrders(101, '2006-01-01', '2008-01-01');
SELECT EMPL_NUM, NAME, dbo.CountSalesrepsOrders(EMPL_NUM, '2006-01-01', '2008-01-01') FROM SALESREPS;
--PRINT @rep;
print 'Result ' + cast(@rep as varchar(10));


--вернет -1, т.к.нет такого покупателя
DECLARE @rep int;
SET @rep = [dbo].CountSalesrepsOrders(100, '2006-01-01', '2008-01-01');
--PRINT @rep;
print 'Result ' + cast(@rep as varchar(10));
...
--DROP FUNCTION CountProductsFromOrders;

create function CountProductsFromOrders () returns int
begin
	declare @rc int = (SELECT COUNT(PRODUCT) FROM ORDERS WHERE AMOUNT > 30000);
	return @rc;
end
go

--use CountProducts function
declare @count_prod int = dbo.CountProductsFromOrders();
print 'count_prod = ' + cast(@count_prod as varchar(10));
go

--Создание функции CountProdFromProducts
create function CountProdFromProducts () returns int
begin
	declare @rc int = (select count(PRODUCT_ID) from PRODUCTS where PRICE > 2000);
	return @rc;
end
go

declare @count_prod int = dbo.CountProdFromProducts();
print 'count products = ' + cast(@count_prod as varchar(10));
go

--DROP FUNCTION CountProdFromProducts;


--3.3.Подсчитать количество заказанных товаров для определенного производителя.

create function OrdersNumber () returns int
begin 
	declare @rc int = (select count(ORDER_NUM) from orders where mfr = 'ACI')
	return @rc;
end
go

declare @count_order int = [dbo].OrdersNumber();
print 'count_orders = ' + cast(@count_order as varchar(10));
go

drop function OrdersNumber;
